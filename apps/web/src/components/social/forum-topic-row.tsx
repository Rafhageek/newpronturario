...
import type { FeedPost } from '@vidalog/core';
import { timeAgo } from '@/lib/time';

export function ForumTopicRow({ topic, basePath }: { topic: FeedPost; basePath: string }) {
  return (
    <Link
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="flex items-center gap-1.5 text-sm font-semibold text-fg">
          {topic.is_pinned && <Pin className="h-3.5 w-3.5 text-amber-300" />}
          {topic.is_solved && <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" />}
