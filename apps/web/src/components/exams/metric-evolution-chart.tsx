...
/** Evolução temporal de uma métrica vs. exames anteriores. */
export function MetricEvolutionChart({ history }: { history: ExamMetric[] }) {
  ...
  const trendColor = last === first ? '#94A3B8' : '#38BDF8';
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={points} margin={{ top: 4, right: 4, bottom: 0, left: -24 }}>
          {ref?.reference_min != null && ref?.reference_max != null && (
            <ReferenceArea y1={ref.reference_min} y2={ref.reference_max} fill="#10B981" fillOpacity={0.08} />
          )}
          <XAxis dataKey="date" tick={{ fill: '#64748B', fontSize: 10 }} tickLine={false} axisLine={false} />
          <YAxis tick={{ fill: '#64748B', fontSize: 10 }} tickLine={false} axisLine={false} width={32} />
          <Line type="monotone" dataKey="value" stroke={trendColor} strokeWidth={2} dot={{ r: 2.5 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
