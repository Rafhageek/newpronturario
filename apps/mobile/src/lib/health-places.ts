 ...
 *
 * A tabela é nova e ainda não está nos tipos gerados do Supabase, então o
 * acesso é feito via cast controlado (typedTable) até regenerar os tipos.
 */
...
export async function createHealthPlace(client: SupabaseClient, input: HealthPlaceInput): Promise<void> {
...
export async function deleteHealthPlace(client: SupabaseClient, id: string): Promise<void> {
...
export async function geocodeAddress(client: SupabaseClient, address: string): Promise<GeocodeResult> {
  const { data, error } = await client.functions.invoke('geocode-address', { body: { address } });

  if (error) {
    // Tenta ler o corpo do erro p/ dizer a causa real (em vez de "não encontrado").
